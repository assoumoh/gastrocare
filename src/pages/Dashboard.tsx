...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Fetch uniquement les 10 derniers patients (pour les stats récentes et la map)
    ...
    // Compteur total patients (listener léger — on ne charge pas les docs)
    const unsubAllPatients = onSnapshot(
      ...
      (snapshot) => {
        ...
        // Enrichir la patientsMap avec tous les patients (pour résoudre les noms)
        const pMap: Record<string, any> = {};
        snapshot.docs.forEach(d => { pMap[d.id] = d.data(); });
        setPatientsMap(pMap);
      }
    );

    // Fetch Appointments (aujourd'hui uniquement)
    ...
    // Fetch Consultations (aujourd'hui uniquement — plus limit(50) sur tout l'historique)
    ...
    // Fetch Exams (5 derniers uniquement)
    ...
    // Fetch Prescriptions (5 dernières uniquement)
    ...
    // Fetch Documents (5 derniers uniquement)
  ...
  // Derived stats
  const preConsultations = consultations.filter(c => c.statutConsultation === 'pre_consultation');
  const enAttente = consultations.filter(c => c.statutConsultation === 'en_attente');
  const enCours = consultations.filter(c => c.statutConsultation === 'en_cours');
  const terminees = consultations.filter(c => c.statutConsultation === 'terminee');
        ...
        <div className="bg-white shadow rounded-lg border border-slate-200 flex flex-col">
          ...
          <div className="p-0 flex-1 overflow-auto">
            <ul className="divide-y divide-slate-200">
              ...
              {exams.length === 0 && (
                <li className="p-6 text-center text-slate-500 text-sm">Aucun examen récent.</li>
              )}
            </ul>
          </div>
        </div>
          ...
          <div className="p-0 flex-1 overflow-auto">
            <ul className="divide-y divide-slate-200">
              ...
              {prescriptions.length === 0 && (
                <li className="p-6 text-center text-slate-500 text-sm">Aucune ordonnance récente.</li>
              )}
            </ul>
          </div>
        </div>
