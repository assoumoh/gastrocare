...
import { collection, query, onSnapshot, orderBy, limit, doc, updateDoc, getDoc, where } from 'firebase/firestore';
import { db } from '../firebase';
import { Link } from 'react-router-dom';
import { Plus, Search, ChevronRight, Edit, Trash2, Eye } from 'lucide-react';
...
export default function Patients() {
  ...
  useEffect(() => {
    // Fetch all patients and filter active ones in memory
    // because where('deleted', '!=', true) excludes documents that don't have the 'deleted' field
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {filteredPatients.map((patient) => (
                <tr key={patient.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {patient.allergies ? (
                      <span className="inline-flex items-center rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800" title={patient.allergies}>
                        Oui
                      </span>
                    ) : (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end space-x-2">
                      ...
                      <button 
                        ...
                        className="text-slate-600 hover:text-slate-900 p-1 rounded-full hover:bg-slate-100"
                        title="Modifier"
                      >
                  ...
                  <div className="mt-2">
                    ...
                    <div className="mt-4">
                      <label htmlFor="deleteCode" className="block text-sm font-medium text-slate-700">
                        Code confidentiel Administrateur
                      </label>
                      <input
                        type="password"
                        name="deleteCode"
