...
export default function PatientExams({ patientId, patientName = 'Patient' }: PatientExamsProps) {
  ...
  useEffect(() => {
    // Essayer avec orderBy, fallback sans si l'index manque
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.warn('Index manquant pour exams, fallback sans orderBy:', error.message);
  ...
  useEffect(() => {
    const qPayments = query(collection(db, 'payments'), where('patient_id', '==', patientId));
  ...
  useEffect(() => {
    const qDocs = query(collection(db, 'documents'), where('patient_id', '==', patientId));
    ...
    <div className="space-y-4">
      ...
      {filteredExams.length === 0 ? (
        <div className="text-center py-12 bg-slate-50 rounded-lg border-2 border-dashed border-slate-300">
          ...
          {appUser?.role !== 'assistante' && (
            <p className="mt-1 text-sm text-slate-500">Commencez par ajouter un nouvel examen pour ce patient.</p>
          )}
        </div>
      ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`ml-2 px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${STATUS_COLORS[status] || 'bg-slate-100 text-slate-800'}`}>
                          {STATUS_LABELS[status] || status}
                        </span>
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3 text-sm text-slate-500">
                        <div className="flex items-center"><Calendar className="mr-1.5 h-4 w-4 text-slate-400" /><span>Demandé: {exam.dateDemande || exam.date_examen}</span></div>
                      ...
                      <div className="mt-2 sm:flex sm:justify-between">
                        <div className="sm:flex flex-col space-y-1">
                          <p className="flex items-center text-sm text-slate-500"><span className="font-medium mr-1">Lieu:</span> {exam.lieu || 'Non spécifié'}</p>
                          <p className="flex items-center text-sm text-slate-500"><span className="font-medium mr-1">Prescripteur:</span> {exam.medecin_prescripteur || 'Non spécifié'}</p>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 pt-3 border-t border-slate-100">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center">
                            <Paperclip className="h-3 w-3 mr-1" />Documents joints ({examDocs.length})
                          </h4>
                        ...
                        {examDocs.length > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {examDocs.map((d) => (
                              <a key={d.id} href={d.url_fichier || d.fileUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-2.5 py-1.5 border border-slate-200 shadow-sm text-xs font-medium rounded text-indigo-700 bg-white hover:bg-slate-50">
                                <FileText className="mr-1.5 h-3 w-3 text-indigo-500" />{d.titre || d.nom || d.type_document || 'Document'}
                              </a>
                            ))}
                          </div>
                        )}
                          ...
                          <div className="space-y-2">
                            {examPayments.map((payment) => (
                              <div key={payment.id} className="flex justify-between items-center text-sm bg-white p-2 rounded border border-slate-100">
                                ...
                                <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium capitalize ${(payment.statut_paiement === 'payé' || payment.statut_paiement === 'réglé') ? 'bg-green-100 text-green-800' : (payment.statut_paiement === 'non payé' || payment.statut_paiement === 'en_attente') ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-800'}`}>
                    ...
                    {appUser?.role !== 'assistante' && (
                      <div className="ml-5 flex-shrink-0 flex space-x-2">
                        ...
                        <button onClick={() => handleDelete(exam.id)} className="p-2 text-slate-400 hover:text-red-600 rounded-full hover:bg-red-50">
